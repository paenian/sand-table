...
clamp_width = m5_rad*2 + wall;
clamp_height = 25;
screw_sep = clamp_width;
clamp_screw_rad = 2;
clamp_screw_cap_rad = 4;
clamp_screw_cap_height = 2;

clamp_base();

translate([0,0,clamp_height]) clamp_lever();
    ...
    difference(){
        union(){
            ...
            hull(){
                ...
                for(i=[0,1]) mirror([0,i,0]) translate([0,screw_sep,clamp_height-clamp_width/2])
        ...
        for(i=[0,1]) mirror([0,i,0]) translate([0,screw_sep,clamp_height-clamp_width/2]) {
            ...
            rotate([0,-90,0]) cylinder(r1=clamp_screw_rad, r2=clamp_screw_cap_rad, h=clamp_screw_cap_height+.05);
            rotate([0,-90,0]) translate([0,0,clamp_screw_cap_height]) cylinder(r=clamp_screw_cap_rad, h=50);
        }
        
        //v-top for aligning
        translate([0,0,clamp_height]) scale([1,2,1]) rotate([0,90,0]) cylinder(r=clamp_screw_rad, h=50, center=true, $fn=4);
...
module clamp_lever(){
    ...
    difference(){
        union(){
            hull(){
                translate([clamp_width,0,0]) cylinder(r1=clamp_width/2-chamfer, r2=clamp_width/2, h=chamfer+.01);
                translate([-clamp_width/2,0,0]) cylinder(r1=clamp_width/2-chamfer, r2=clamp_width/2, h=chamfer+.01);
                
                translate([clamp_width,0,chamfer]) cylinder(r=clamp_width/2, h=wall-chamfer);
                translate([-clamp_width/2,0,chamfer]) cylinder(r=clamp_width/2, h=wall-chamfer);
            }
            
            intersection(){
                translate([0,0,wall]) scale([1,2,1]) rotate([0,90,0]) cylinder(r=clamp_screw_rad, h=50, center=true, $fn=4);
                hull(){
